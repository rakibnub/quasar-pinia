...
export const authStore = defineStore("auth", {
  ...
  actions: {
    ...
    async DO_LOGIN(payload) {
      await api.post("/api/login/", payload).then(async (response) => {
        this.SET_TOKEN(response.data);
        await this.GET_USER(response.data.token);
      });
    },
    async GET_USER(token) {
      await api.get("/api/user/me/", { token: token }).then((response) => {
    ...
    async CHECK_TOKEN() {
      ...
      if (!token) return Promise.reject(new Error("Token valid!"));
      this.SET_RELOAD_TOKEN(token);
